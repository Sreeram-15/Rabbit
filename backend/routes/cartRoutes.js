...
// Helper function to get a cart by userId or guset Id
...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    if (cart) {
      ...
      if (productIndex > -1) {
        // If the product already exists, we can update the quantity
  ...
  try {
    ...
    if (productIndex > -1) {
      ...
      if (quantity > 0) {
        ...
        cart.products.splice(productIndex, 1); //Remove if product quantity is zero
      }
      cart.totalPrice = cart.products.reduce(
        (acc, item) => acc + item.price * item.quantity,
        0
...
// @route GET api/cart/
// @desc Get logged-in user or guues user's Cart
// @access public
